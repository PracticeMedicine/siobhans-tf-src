...
ConVar mp_autoteambalance( "mp_autoteambalance", "2", FCVAR_NOTIFY, "Automatically balance the teams based on mp_teams_unbalance_limit. 0 = off, 1 = forcibly switch, 2 = ask volunteers", true, 0, true, 2 );
...
void CTeamplayRoundBasedRules::State_Think_TEAM_WIN( void )
{
	if ( gpGlobals->curtime > m_flStateTransitionTime )
	{
		...
		else if ( IsInTournamentMode() || !TFGameRules()->IsMannVsMachineMode())
		{
