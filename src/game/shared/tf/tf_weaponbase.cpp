...
ConVar sv_defaultdeployspeed("sv_defaultdeployspeed", "1.34", FCVAR_REPLICATED | FCVAR_NOTIFY, "Weapon will deploy at this rate.");
...
bool CTFWeaponBase::Deploy( void )
{
	...
	if ( bDeploy )
	{
		...
		float flWeaponSwitchTime = (0.5f / sv_defaultdeployspeed.GetFloat()) * 1.34f;
		...
		float flPlaybackRate = sv_defaultdeployspeed.GetFloat() / flDeployTimeMultiplier;
