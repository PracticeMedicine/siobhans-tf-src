...
ConVar  tf_mortar_allow_fulltracking("tf_mortar_allow_fulltracking", "0.0", FCVAR_REPLICATED | FCVAR_CHEAT, "Enable to allow full tracking / infinte redirects for Mortar Launcher");
...
void CTFCrossbow::SecondaryAttack( void )
{
	...
	int iMilkBolt = 0;
	CALL_ATTRIB_HOOK_INT( iMilkBolt, set_weapon_mode );
	if ( iMilkBolt )
	{
		CTFPlayer *pPlayer = GetTFPlayerOwner();
		if ( !pPlayer )
