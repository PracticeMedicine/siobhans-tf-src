...
void CTFMinigun::WeaponSoundUpdate()
{
	...
	if ( flSpeed != 1.0f )
	{
		flPitch = RemapValClamped( flSpeed, 1.5f, 0.5f, 80.f, 125.f );
	}
	...
	int iTFCMinigunPitch = -1;
	CALL_ATTRIB_HOOK_INT(iTFCMinigunPitch, tfc_minigun_pitch);
	if (iTFCMinigunPitch > 0) {
		flPitch /= 0.8;
	}
