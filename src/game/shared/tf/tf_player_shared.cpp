...
const char *CTFPlayer::GetOverrideStepSound( const char *pszBaseStepSoundName )
{
	...
	if( (TFGameRules() && TFGameRules()->IsMannVsMachineMode() && GetTeamNumber() == TF_TEAM_PVE_INVADERS && !IsMiniBoss() && !m_Shared.InCond( TF_COND_DISGUISED )) || (TFGameRules() && TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS && !IsMiniBoss() && !m_Shared.InCond(TF_COND_DISGUISED)) )
	{
		return "MVM.BotStep";
	}
