...
// 							NDebugOverlay::Sphere( info.GetDamagePosition(), tf_space_thrust_scout.GetFloat(), 255, 20, 20, true, 5.f );
	...
	if ( bBleed && ( realDamage > 0.f ) )
	{
		...
		if ( TFGameRules() && TFGameRules()->IsMannVsMachineMode() && GetTeamNumber() == TF_TEAM_PVE_INVADERS || TFGameRules() && TFGameRules()->IsRaidMode() && GetTeamNumber() == TF_TEAM_PVE_DEFENDERS)
		{
...
// 		}
	...
	if ( TFGameRules()->IsMannVsMachineMode() || TFGameRules()->IsRaidMode() )
	{
...
{
	CTFPlayer* pPlayer = ToTFPlayer(UTIL_GetCommandClient());
	if (sv_cheats->GetInt() != 0 || g_pDeveloper->GetInt() > 0 ) // || pPlayer->IsDeveloper() )
	{
		if (!pPlayer)
			return;
		...
		BotGenerateAndWearItem(pPlayer, args[1]);
	}
	else
	{
		Msg("Can't use cheat command give_econ in multiplayer, unless the server has 'sv_cheats' or 'developer' set to 1.\n");
		return;
	}
}
