...
float ModifyBossDamageOld( const CTakeDamageInfo &info )
{
...
int CBotNPC::OnTakeDamage_Alive( const CTakeDamageInfo &rawInfo )
{
	...
	info.SetDamage( ModifyBossDamageOld( info ) );
...
//---------------------------------------------------------------------------------------------
void PushawayPlayerOld( CTFPlayer *victim, const Vector &pushOrigin, float pushForce )
{
	if ( !victim )
		return;
...
ActionResult< CBotNPC >	CBotNPCRush::Update( CBotNPC *me, float interval )
{
	...
	for( int i=0; i<playerVector.Count(); ++i )
	{
		...
		PushawayPlayerOld( victim, closestPointOnChargePath, pushForce );
...
ActionResult< CBotNPC >	CBotNPCGetOffMe::Update( CBotNPC *me, float interval )
{
	if ( m_timer.IsElapsed() )
	{
		...
		if ( me->GetAttachment( "head", headPos, headAngles ) )
		{
			...
			{
				// push 'em off
				PushawayPlayerOld( onMeVector[i], headPos, tf_bot_npc_charge_pushaway_force.GetFloat() );
			}
		}
