...
void BotGenerateAndWearItem( CTFPlayer *pBot, const char *itemName )
{
	...
	int iClass = pBot->GetDesiredPlayerClassIndex();
	...
	if (pItem)
	{
		bool isBot = pBot->IsBot();
		if (!static_cast<CEconEntity*>(pItem)->GetAttributeContainer()->GetItem()->GetStaticData()->CanBeUsedByClass(iClass) && !isBot) return;
