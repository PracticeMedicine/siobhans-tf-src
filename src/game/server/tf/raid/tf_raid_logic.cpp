...
CTFBot* SpawnRedTFBot(int botclass, const Vector& spot) {
	...
	CTFBot *newBot = NextBotCreatePlayerBot< CTFBot >("TFBot");

	if (!newBot)
		return NULL;

	newBot->HandleCommand_JoinTeam("red");
	newBot->HandleCommand_JoinClass(GetPlayerClassData(botclass)->m_szClassName);
	newBot->AllowInstantSpawn();
	newBot->SetPosition(spot);
	newBot->SetAttribute(CTFBot::REMOVE_ON_DEATH);
	newBot->GetPlayerClass()->SetCustomModel(g_szBotModels[botclass], USE_CLASS_ANIMATIONS);
	newBot->UpdateModel();
...
//--------------------------------------------------------------------------------------------------------
class CRaidPopulator : public ISearchSurroundingAreasFunctor
{
public:
	CRaidPopulator( float leaderIncursionRange, int maxWanderersToSpawn )
	{
...
void CRaidLogic::Update( void )
{
	...
	// populate wanderers
	CRaidPopulator populator( maxIncursion, tf_raid_max_wanderers.GetInt() - m_wandererCount );
