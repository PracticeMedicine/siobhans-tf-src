...
void CTFBot::GiveRandomItem( loadout_positions_t loadoutPosition )
{
	...
	FOR_EACH_MAP_FAST( mapItemDefs, i )
	{
		...
		if ( pItemDef && pItemDef->GetLoadoutSlot( GetPlayerClass()->GetClassIndex() ) == loadoutPosition && pItemDef->GetQuality() == AE_UNIQUE )
		{
	...
	for ( int w=0; w<MAX_WEAPONS; ++w )
	{
		...
		else if ( weapon->GetWeaponID() == TF_WEAPON_LUNCHBOX )
		{
			...
			if ( lunchbox->HasAmmo() )
			{
				...
				if ((!IsPlayerClass(TF_CLASS_SCOUT) && HealthFraction() < tf_bot_health_ok_ratio.GetFloat()) || (IsPlayerClass(TF_CLASS_SCOUT) && m_Shared.GetScoutEnergyDrinkMeter() >= 100))
				{
