...
ActionResult< CTFBot >	CTFBotSeekAndDestroy::Update( CTFBot *me, float interval )
{
	...
	else
	{
		...
		if (TFGameRules()->GetGameType() == TF_GAMETYPE_CP && !TFGameRules()->IsRaidMode() && !TFGameRules()->RoundHasBeenWon() && me->GetTimeLeftToCapture() < tf_bot_offense_must_push_time.GetFloat())
		{
